...
open class GetCharactersUseCaseMock: GetCharactersUseCase, Mock {
    ...
    open func callAsFunction() -> AnyPublisher<[Marvel.Character], Error> {
        addInvocation(.m_callAsFunction)
		let perform = methodPerformValue(.m_callAsFunction) as? () -> Void
		perform?()
		var __value: AnyPublisher<[Marvel.Character], Error>
		do {
    ...
    open class Given: StubbedMethod {
        ...
        public static func callAsFunction(willReturn: AnyPublisher<[Marvel.Character], Error>...) -> MethodStub {
            return Given(method: .m_callAsFunction, products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func callAsFunction(willProduce: (Stubber<AnyPublisher<[Marvel.Character], Error>>) -> Void) -> MethodStub {
            let willReturn: [AnyPublisher<[Marvel.Character], Error>] = []
			let given: Given = { return Given(method: .m_callAsFunction, products: willReturn.map({ StubProduct.return($0 as Any) })) }()
			let stubber = given.stub(for: (AnyPublisher<[Marvel.Character], Error>).self)
			willProduce(stubber)
			return given
        }
    }
